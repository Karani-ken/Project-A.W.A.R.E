...
10. Dead Letter Queue (DLQ) - A dead letter queue is a queue that stores messages that cannot be processed successfully by the consumer. When a message is rejected or fails to be processed, it can be sent to a dead letter queue for further analysis or reprocessing.
11. Dead Letter Exchange (DLX) - A dead letter exchange is an exchange that routes messages to a dead letter queue. When a message is rejected or fails to be processed, it can be sent to a dead letter exchange, which will then route it to the appropriate dead letter queue for further analysis or reprocessing.
12. x-dead-letter-exchange - This is a queue argument that specifies the dead letter exchange to which messages should be sent when they are rejected or fail to be processed. It allows you to configure the routing of messages to a dead letter queue for further analysis or reprocessing.
13. fanout exchange - A fanout exchange is a type of exchange that routes messages to all bound queues indiscriminately. When a message is published to a fanout exchange, it is delivered to all queues that are bound to that exchange, regardless of any routing keys or patterns. This is useful for broadcasting messages to multiple consumers simultaneously.
